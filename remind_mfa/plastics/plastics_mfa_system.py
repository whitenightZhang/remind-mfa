...
class PlasticsMFASystemFuture(fd.MFASystem):
    ...
    def compute_flows(self):
        ...
        fabrication_to_use = (
            stk["in_use"].inflow.values - flw["final_imports => use"].values
        )
        neg_mask = fabrication_to_use < 0
        if np.any(neg_mask):
            flw["good_market => final_imports"].values[neg_mask] = stk["in_use"].inflow.values[neg_mask]
            flw["final_imports => use"].values[neg_mask] = stk["in_use"].inflow.values[neg_mask]
            fabrication_to_use[neg_mask] = 0.0

        flw["fabrication => final_exports"][...] = flw["final_exports => good_market"][...]

        flw["fabrication => use"][...] = fabrication_to_use
